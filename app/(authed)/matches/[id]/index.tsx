...
// Rating UI moved to `app/(authed)/matches/[id]/rate.tsx` (2026-05-14). The
// detail screen now just exposes a "Maçı puanla" pill that links there.
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <ScrollView
        ...
        {/* 2026-05-14: rating UI moved to a dedicated screen so the detail
            page stays focused. Active participants get a button into it
            during RATING_WINDOW and CLOSED. Backend also fires an
            auto-notification on RATING_WINDOW entry that deep-links here. */}
        {showRateEntry && myActiveSlot ? (
          <Pressable
            onPress={() =>
              router.push(`/matches/${matchId}/rate` as Href)
            }
            ...
            <Text className="text-blue-600 font-semibold">
              {tr.matchDetail.rating.openPill}
            </Text>
          </Pressable>
        ) : null}
...
// RatingsSection / RatingCard / ScorePicker / StarPicker moved to
// `app/(authed)/matches/[id]/rate.tsx` (2026-05-14).
